...
from py_attacks.utils import calc_l2distsq
from py_attacks.utils import tanh_rescale
from py_attacks.utils import torch_arctanh
from py_attacks.utils import clamp
from py_attacks.utils import to_one_hot
from py_attacks.utils import replicate_input

from py_attacks.base import Attack
from py_attacks.base import LabelMixin
from py_attacks.utils import is_successful
...
class CarliniWagnerL2Attack(Attack, LabelMixin):
    ...
    def _forward_and_update_delta(
        ...
        # ----------------------------------
        output = self.predict(adv, y)
        pred = output['logit_x'] # output['pred']

        l2distsq = calc_l2distsq(adv, transimgs_rescale)
        loss = self._loss_fn(pred, y_onehot, l2distsq, loss_coeffs)
        loss.backward()
        optimizer.step()

        return loss.item(), l2distsq.data, pred.data, adv.data
        # ----------------------------------
    ...
    def perturb(self, x, y=None):
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            for ii in range(self.max_iterations):
                loss, l2distsq, output, adv_img = self._forward_and_update_delta(optimizer, x_atanh, delta, y_onehot, loss_coeffs, y)
                if self.abort_early:
